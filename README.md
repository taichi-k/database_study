...
MVCCにより、複数バージョンを保持しながら同時実行を行う。そのため、VACUUMでゴミ回収が必須。
高機能なので、大規模システムではMySQLよりもPostgresの方が向いている場面があるが、設定やチューニングが必要。この辺がLearning Cost。
...
Key-Valueストアだが、かなりいろんなデータ型を格納できる（ListとかSetとかHashとか）。
Pub/Subの機能やStream、スコア付き集合など、特定のユースケースに特化した機能がある。
